...
export async function PUT(req: Request) {
    try {
        ...
        const coupon = await Coupon.find({ _id: { $in: ids } }).lean()
        if (!coupon.length) {
        ...
        if (deleteType === 'SD') {
            await Coupon.updateMany({ _id: { $in: ids } }, { $set: { deletedAt: new Date().toISOString() } })
        } else {
            await Coupon.updateMany({ _id: { $in: ids } }, { $set: { deletedAt: null } })
        }
