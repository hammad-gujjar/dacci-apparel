...
export async function GET(req: Request) {
    try {
        ...
        const getCoupon = await Coupon.find(filter).select('-__v').sort({ createdAt: -1 }).lean()

        if(!getCoupon) {
            return NextResponse.json({ success: false, statusCode: 404, message: 'Collection empty' });
        }

        return NextResponse.json({ success: true, statusCode: 200, message: 'Collection has been export', data:getCoupon });
