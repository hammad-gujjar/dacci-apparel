...
export async function PUT(req: Request) {
    try {
        ...
        const review = await Review.find({ _id: { $in: ids } }).lean()
        if (!review.length) {
        ...
        if (deleteType === 'SD') {
            await Review.updateMany({ _id: { $in: ids } }, { $set: { deletedAt: new Date().toISOString() } })
        } else {
            await Review.updateMany({ _id: { $in: ids } }, { $set: { deletedAt: null } })
        }
